...
* ISP 引入一个显著性过滤器，令 $t_{pi}$ 为上次传输 第 $i$ 个参数时的时间，定义每个参数的显著性过滤器为：
  $$
  \left|\frac{\sum_{t^{'}=t_{pi}}^{t}u_{i,t'}}{x_{i,t}} \right| \gt v_t
  $$

* 为了确保每个步骤的算法进度相等，ISP 允许用户调整显著性过滤器的严格程度，以达到最佳点。如果原始阈值为 $v$, 然后在第 $t$ 个步长阈值是 $v_t$ ，ML 算法的计算公式如下所示：$v_t=\frac{v}{\sqrt{t}}$。
...
* 根据历史损失信息，调度器首先检测收敛速度中的“拐点”，超过这个时间点之后损失减少速度显著减慢，并使用此时损失值的历史来拟合参考训练损失曲线 $L_P(t)$。 调度程序将使用该曲线来量化未来 worker 被移除所导致的与原始收敛速度的偏差。

* 调度器在每个调度间隔上重复以下操作序列：

  * 估计阶段：使用自上次移除 worker 以来收集的损失值，拟合一条新的的训练损失曲线 $l_p(t)$。

  * 决策阶段：调度器根据时间 $\Delta$ 内预计损失减少的相对误差，即 $s_\Delta(t) < S$ , 决定移除一个新的 worker， 其中时间范围 $\Delta$ 内预计损失减少的相对误差为：
    $$
    s_{\Delta}(t):=\left[\frac{L_P(t+\lfloor \frac{\Delta}{d_P} \rfloor)-l_p(t+\lfloor \frac{\Delta}{d_p} \rfloor)}{L_P(t+\lfloor \frac{\Delta}{d_P} \rfloor)}\right]
    $$
